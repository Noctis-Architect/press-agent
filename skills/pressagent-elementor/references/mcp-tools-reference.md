...
## ۴. ابزار بازرسی بصری و تصویربرداری (Visual Inspection)

### `pressagent_capture_screenshot`
تصویربرداری واقعی از برگه وردپرس با استفاده از مرورگر بدون سر (Headless Chromium) جهت مشاهده خروجی بصری و اصلاح باگ‌های ظاهری.

- **پارامترها:**
  - `page_id` (number, optional): شناسه برگه وردپرس
  - `url` (string, optional): مسیر برگه (مانند `"/"` یا `"/cart/"`)
  - `viewport` (string, optional): ابعاد تصویربرداری (`"desktop"` پیش‌فرض با رزولوشن 1280x800، `"mobile"` با رزولوشن 375x812، یا `"tablet"` با 768x1024)
  - `full_page` (boolean, optional): عکس تمام‌صفحه با اسکرول کامل (پیش‌فرض false)
  - `wait_ms` (number, optional): مدت انتظار بر حسب میلی‌ثانیه برای لود کامل فونت‌ها و انیمیشن‌ها (پیش‌فرض 2500)
- **خروجی:**
  مسیر فایل تصویر ذخیره‌شده (مثلاً `/tmp/pressagent_desktop_1788829900.png` و پیوند نمادین `/tmp/pressagent_preview_desktop.png`).
- **نحوه استفاده در ایجنت:**
  ایجنت پس از دریافت مسیر فایل، ابزار `view_file` را روی فایل تصویر فراخوانی می‌کند تا ظاهر واقعی صفحه، چینش افقی/عمودی المان‌ها و ریسپانسیو بودن را مستقیماً ببیند و در صورت نیاز خودکار اصلاح کند.
